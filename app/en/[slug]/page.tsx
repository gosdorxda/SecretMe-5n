...
import { SendMessageForm } from "@/app/[slug]/send-message-form"
...
// Modified static metadata for English SEO
export const metadata = {
  title: "User Profile | SecretMe",
  description: "Send anonymous messages through SecretMe",
}

// List of demo usernames that can't receive messages
...
export default async function ProfilePage({ params }: { params: { slug: string } }) {
  ...
  // Use getVerifiedUser to get the verified user
  ...
  if (!isNaN(numericId)) {
    ...
    // Fix: Remove additional condition, all users with valid numeric_id can be accessed
    if (userByNumericId) {
      // If the user is premium and has a username, redirect to the username URL
      // Preserve the language prefix in the redirect
      if (userByNumericId.is_premium && userByNumericId.username) {
        return redirect(`/en/${userByNumericId.username}`)
      }
    ...
    // Only premium users can set a username
  ...
  let viewCount = 0
  try {
    // Check if view data already exists
    ...
    if (viewError && viewError.code !== "PGRST116") {
      ...
      // If no view data exists, create new
      ...
      // If view data exists, increment by 1
      ...
      <div className="w-full max-w-[56rem] mx-auto py-4 sm:py-8 px-4">
        ...
        <div className="flex flex-col items-center text-center mb-8">
          ...
          <div className="relative mb-4">
            ...
            {/* Premium badge attached to border */}
            {user.is_premium && (
          ...
          {/* Bio with show more feature - premium only */}
          ...
          <div className="flex flex-col items-center gap-3">
            ...
            {/* Button to share profile as image - for all users */}
            <ProfileImageButton
        ...
        {!user.is_premium && (
          <div className="mb-6">
            <PremiumFeatureTeaser locale="en" />
          </div>
        )}

        {/* Send Message Form or Demo Notice */}
        <div className="px-0 sm:px-0">
          {isDemo ? (
              ...
              <CardContent className="p-6">
                <div className="flex flex-col items-center text-center space-y-4">
                  ...
                  <div className="space-y-2">
                    <h3 className="text-lg font-medium">This is a Demo Profile</h3>
                    <p className="text-sm text-gray-600">
                      Message sending is disabled for this demo account. Check out the available premium features!
                    </p>
                  </div>
                  <div className="pt-2">
                    <Button className="bg-amber-500 hover:bg-amber-600" size="sm" asChild>
                      <a href="/en/features">View Premium Features</a>
                    </Button>
                  </div>
                </div>
              </CardContent>
            ...
            <SendMessageForm user={user} locale="en" />
          )}
        </div>
        ...
        <div className="mt-12">
          ...
          >
            <div className="flex items-center">
              Messages for @{user.username || user.numeric_id}
          ...
          <MessageList
            ...
            displayName={user.name} // Add displayName to MessageList
            locale="en" // Set locale to English
          />
        </div>
      </div>
      <ProfileCta locale="en" />
    </div>
  )
}
